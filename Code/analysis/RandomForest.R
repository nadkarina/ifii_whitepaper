...
variables_with_labels = map(raw_fii_dataset, function(x) attr(x, "class") == "haven_labelled") %>% 
  unlist() %>% 
  names()

clean_fii_dataset = raw_fii_dataset %>%
  mutate_at(vars(variables_with_labels), as_factor)

# Remove unnecessary vars
#clean_fii_dataset <- select(clean_fii_dataset, -c(know_mobilemoney, use_mobilemoney, everuse, use_6mon_atm_bin, weight))  
clean_fii_dataset <- select(clean_fii_dataset, -c(everuse, use_6mon_atm_bin, weight))  

# Subset by Gender
clean_fii_dataset <- clean_fii_dataset
clean_fii_dataset_f <- subset(clean_fii_dataset, female == "Yes")
clean_fii_dataset_m <- subset(clean_fii_dataset, female == "No")
...
rf_pred_hot <- predict(rf_model_hot, matrix_test_hot, positive="Yes")
cm_rf_hot <- confusionMatrix(rf_pred_hot[["predictions"]], as.factor(matrix_test_outcome),positive="Yes") 
cm_rf_hot

rf_pred_hot_f <- predict(rf_model_hot_f, matrix_test_hot_f, positive="Yes")
cm_rf_hot_f <- confusionMatrix(rf_pred_hot_f[["predictions"]], as.factor(matrix_test_outcome_f),positive="Yes") 
cm_rf_hot_f

rf_pred_hot_m <- predict(rf_model_hot_m, matrix_test_hot_m, positive="Yes")
cm_rf_hot_m <- confusionMatrix(rf_pred_hot_m[["predictions"]], as.factor(matrix_test_outcome_m),positive="Yes") 
cm_rf_hot_m
